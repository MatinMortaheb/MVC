...
PicEncoder::xInitSPS( Bool bAVCSPS )
{
 ...
 if( !bAVCSPS)  
  ...
  uiDPBSize = 4+ mMx (2, (1 <<(m_pcCodingParameter->getDecompositionStages()-1)) + m_pcCodingParameter->getDecompositionStages());
  if (!bAVCSPS)
  ...
  if( (m_pcCodingParameter->getMbAff() != 0 || m_pcCodingParameter->getPAff() != 0 ))
	  ...
	  rpcSPS->setNumRefFrames ( mMn (16, uiMaxFramesInDPB/mvcScaleFactor) -  m_uiGOPSize/2); //Need to consider non-ref frames cslim 130909
  else
	  rpcSPS->setNumRefFrames ( mMn (16, uiMaxFramesInDPB/mvcScaleFactor) ); 
...
PicEncoder::xSetRplrAndMmco( FrameSpec& rcFrameSpec )
{
  ...
  UInt uiNeedLowPassBefore = mMx( 1, rcFrameSpec.getNumRefIdxActive( LIST_0 ) );
