...
void updateCropParametersFromFile(ResizeParameters * rp, FILE * crop_file, int method, char* name)
{
  ...
  print_usage_and_exit ((rp->m_iOutWidth>max(rp->m_iInWidth,rp->m_iGlobWidth)||rp->m_iOutHeight>max(rp->m_iInHeight,rp->m_iGlobHeight)||rp->m_iOutWidth<mMn(rp->m_iInWidth,rp->m_iGlobWidth)||rp->m_iOutHeight<mMn(rp->m_iInHeight,rp->m_iGlobHeight)),name,"wrong crop window size");
  ...
  while (i<argc)
  {
    if (strcmp(argv[i], "-crop")==0)
    {
      ...
      if (rp->m_iExtendedSpatialScalability==1)
      {
        ...
        print_usage_and_exit ((rp->m_iOutWidth>max(rp->m_iInWidth,rp->m_iGlobWidth)||rp->m_iOutHeight>max(rp->m_iInHeight,rp->m_iGlobHeight)||rp->m_iOutWidth<mMn(rp->m_iInWidth,rp->m_iGlobWidth)||rp->m_iOutHeight<mMn(rp->m_iInHeight,rp->m_iGlobHeight)),argv[0],"wrong crop window size");
