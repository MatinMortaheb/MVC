...
protected:
  typedef struct
  {
    Void init( Short sLimit, Mv& rcMvPel, Mv cMin, Mv cMax)
    {
      ...
      iNegVerLimit = mMn( sLimit, sNegV) - rcMvPel.getVer();
      iPosVerLimit = mMn( sLimit, sPosV) + rcMvPel.getVer();

      Short sPosH = cMax.getHor() - rcMvPel.getHor();
      Short sNegH = rcMvPel.getHor() - cMin.getHor();
      iNegHorLimit = mMn( sLimit, sNegH) - rcMvPel.getHor();
      iPosHorLimit = mMn( sLimit, sPosH) + rcMvPel.getHor();
    }
    Int iNegVerLimit;
    Int iPosVerLimit;
    Int iNegHorLimit;
